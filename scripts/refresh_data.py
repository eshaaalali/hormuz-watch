...
def main():
    ...
    # Always ensure archive.json exists on disk (bootstrapped from combined.json
    # if needed) even on a run where the fetch failed, so it's never a missing
    # file the workflow's git add trips over.
    if fetch_ok or not os.path.exists(ARCHIVE_PATH):
